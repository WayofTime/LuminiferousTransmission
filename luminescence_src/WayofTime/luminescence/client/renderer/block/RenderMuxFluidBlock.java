...
public class RenderMuxFluidBlock extends TileEntitySpecialRenderer
{
	...
	{
		if (tileEntity instanceof TEColouredMultiplexerFluid)
		{
			TEColouredMultiplexerFluid tileSpellBlock = (TEColouredMultiplexerFluid) tileEntity;
			...
			FMLClientHandler.instance().getClient().renderEngine.bindTexture(new ResourceLocation(this.getResourceLocationForMeta(meta)));
			GL11.glPushMatrix();
			...
			for(int i=0; i<6; i++)
			{
				...
				if(tileSpellBlock.getRenderTimeForSide(dir)>0)
				{
					float distance = tileSpellBlock.getDistanceAndSetMultiBeamRender(dir,2);
